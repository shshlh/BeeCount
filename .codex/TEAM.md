...
| 7.13.6 诊断修复：导入后账户管理页缺账户 | ⬜ 待执行 | invest-logic + qa | 导出→清空→导入逐字段断言 + 定位字段写错 + 修复 | — |
| 7.13.7 返工：搜索页转账流水显示对齐流水页 | ⬜ 待执行 | invest-ui + qa | 转账行传 isTransfer/accountName 转出→转入 + 金额无正负号 + 测试 | — |
