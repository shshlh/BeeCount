...
| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.13.1 搜索入口迁移（首页→明细页） | ⬜ 待执行 | invest-ui | 首页移除搜索 + 明细页右上角新增搜索 + 现有按钮顺移 | — |
| 7.13.2 搜索三层筛选 + 转账搜索 | ⬜ 待执行 | invest-ui + invest-logic + qa | 收入/支出/转账 → 一级 → 二级 默认收起 + 转账维度 + 测试 | — |
| 7.13.3 数据管理页重构 | ⬜ 待执行 | invest-ui + qa | 分类与标签 / 导入与导出 / 空间与清理 三组件 | — |
