# 团队协作章程 · Fork & Invest

本文档是本项目的**指挥中心**。任何新会话（团队角色）启动后，第一件事就是读取本文档，了解项目状态、自己的任务和当前进展。

---

## 工作流程

```
PM 创建/指派任务 → 角色线程独立工作 → 任务完成更新本文件
→ PM 审查 → 合入基线 → 进入下一轮
```

**核心规则**：
1. 每个线程（会话）是一个独立团队成员
2. 所有共享状态写在本文件中
3. 任务完成后必须更新本文件的「任务板」状态
4. git 提交作为唯一的代码交付物
5. PM 负责审查合入

**同步机制**：
- 每个任务开始时，阅读本文档 → 确认当前状态
- 每个任务结束时，更新本文档的任务板
- 当会话恢复或上下文切换时，重新读取本文档

---

## 团队成员

| 角色 | 代号 | 责任范围 | 文件所有权 |
|---|---|---|---|
| 项目经理 | PM | 总体规划、任务分派、审查合入、文档维护 | docs/, .codex/ |
| 数据架构师 | architect | Drift schema 设计、迁移脚本、Repository | lib/data/db.dart, lib/data/repositories/ |
| 投资逻辑工程师 | invest-logic | 投资原子事务、Service、Provider | lib/services/data/, lib/providers/ |
| UI 工程师 | invest-ui | 持仓页面、导航改造、交互实现 | lib/pages/investment/, lib/widgets/investment/ |
| 测试工程师 | qa | 单元测试、端到端测试 | test/ |
| 功能集成工程师 | integration | Excel 导入、日历视图、WebDAV | lib/services/import/, lib/pages/calendar/ |
| 数据迁移工程师 | migration | 旧数据导出、CSV 生成、字段映射 | scripts/ |
| 外部审查员 | kimi | 只读代码审查、美化建议 | 无（只读，不修改代码） |

---

## 任务板

### 阶段 7.5：发布前修复与收尾

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.5.1 同名不同币种账户提示优化 | ✅ | architect + invest-logic + invest-ui | 测试修正 + 重复名提示建议 + 回归 | 2026-08-11 |
| 7.5.1 PM 复审合入 | ✅ | PM | HANDOFF 记录 + commit + Android APK 编译 | 2026-08-11 |
| 7.5.2 App 图标更换「托」字书法标 | ✅ | kimi（用户直派，跨只读边界） | 三图层接入 + flutter_launcher_icons 重生成 + inset 修正 + HANDOFF 记录 | 2026-08-11 |
| 7.5.2 PM 复审合入 | ✅ | PM | 像素级复核 + HANDOFF 记录 + 待实机验证清单 | 2026-08-11 |
| 7.5.3 App 内品牌图替换 | ✅ | invest-ui（+ kimi 可选） | BeeIcon 换新标 + logo2 替换 + 隐藏引用清理 + 测试 | 2026-08-11 |
| 7.5.3 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 0 issue + HANDOFF 记录 + APK | 2026-08-11 |
| 7.5.4 基金转换记录一体化 + 转入成本 | ✅ | invest-logic + invest-ui + qa | 转换整批编辑/删除 + 转入成本字段 + B 成本口径 + 测试 | 2026-08-12 |
| 7.5.4 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 无新增 + HANDOFF 记录 + APK | 2026-08-12 |
| 7.5.5 转换流水只留退回 + 基金记录时间编辑 | ✅ | invest-logic + invest-ui + qa + architect | internal 流水排除 + v39 回填 + 时分选择 + 测试 | 2026-08-12 |
| 7.5.5 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 无新增 + HANDOFF 记录 + APK | 2026-08-12 |
| 7.5.6 基金记录编辑日期时间合并 | ✅ | invest-ui + qa | showWheelDateTimePicker 统一 + 移除时间行 + 测试 | 2026-08-12 |
| 7.5.6 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 0 issue + HANDOFF 记录 + APK | 2026-08-12 |
| 7.6.1 删除转换后清理空持仓 | ✅ | invest-logic + qa | deleteConversion 空持仓清理 + 测试 | 2026-08-12 |
| 7.6.2 投资流水明细编辑安全 | ✅ | invest-logic + invest-ui + qa | 通用更新重算持仓 + 受限编辑 + 测试 | 2026-08-12 |
| 7.6.1 PM 复审合入 | ✅ | PM | 全量测试 0 failed + HANDOFF 记录 + APK | 2026-08-12 |
| 7.6.2 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 无新增 + HANDOFF 记录 + APK | 2026-08-12 |
| 7.6.3 受限编辑页标签即时刷新 | ✅ | invest-ui + qa | 保存后触发 tagListRefresh / sync / 统计刷新 + 测试 | 2026-08-12 |
| 7.6.3 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 0 issue + HANDOFF 记录 + APK | 2026-08-12 |
| 7.7.1 版本号 + 正式签名 | ✅ | architect + invest-logic | 1.0.0+1 + release keystore + key.properties + 离线备份 | 2026-08-13 |
| 7.7.2 release 打包回归 + 归档 | ⬜ 待实机验证 | qa + invest-logic | prod release APK 已出 + 全量测试 766 passed + 待实机回归 + v1.0.0 tag/push | — |
| 7.8.1 CSV 导出投资内容 | ✅ | invest-logic + invest-ui + qa | 持仓/投资流水/分组 CSV + 测试（旧退回流水兼容） | 2026-08-13 |
| 7.8.1 返工：旧退回流水兼容 | ✅ | invest-logic + qa | 查询/识别兼容无 batchId 退回 + 测试 | 2026-08-13 |
| 7.8.1 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 无新增 + HANDOFF 记录 + APK | 2026-08-13 |
| 7.8.2 配置 YAML 备份投资数据 | ❌ 取消 | PM | 用户确认投资内容归「导出数据」CSV，配置导出不改 | 2026-08-13 |
| 7.9.1 导出投资与明细一一对应 | ✅ | invest-logic + qa + architect | 普通 CSV 排除投资流水 + 投资 CSV 唯一出处（含流水ID/转出转入账户）+ 测试 | 2026-08-13 |
| 7.9.2 明细流水批量删除 | ✅ | invest-ui + qa + invest-logic | 多选模式 + 批量删除 + 持仓重算 + 测试 | 2026-08-13 |
| 7.9.3 账本初始化/清空全部数据 | ✅ | architect + invest-logic + invest-ui + qa | resetLedger 清流水/持仓/分组/附件 + UI + 测试 | 2026-08-13 |
| 7.9.4 重复导入同一份导出数据去重 | ✅ | architect + invest-logic + qa | 流水ID 列 + syncId 幂等导入 + skipped 统计 + 测试 | 2026-08-13 |
| 7.9.5 明细投资流水显示基金代码/名称 | ✅ | invest-ui + qa + invest-logic | holdingId 映射 + 基金标识 + 不占用备注 + 测试 | 2026-08-13 |
| 7.9 PM 复审合入 | ✅ | PM | 全量测试 0 failed + analyze 无新增 error + HANDOFF 记录 + APK | 2026-08-13 |
| 7.9.3 返工：初始化账本同时清空账户 | ❌ 并入 7.10.1 | PM | 统一由 7.10.1 处理账户级联清理 | 2026-08-13 |
| 7.10.1 账户与账本强绑定 | ⬜ 待执行 | architect + invest-logic + qa | reset/clear/delete 级联删账户 + 清默认偏好 + 测试 | — |
| 7.10.2 导出文件名前缀改名 | ⬜ 待执行 | invest-ui + qa | jizhang_zhushou 前缀 + 导入兼容测试 | — |
| 7.10.3 投资 CSV 可导入 | ⬜ 待执行 | invest-logic + architect + invest-ui + qa | 专用投资导入 + 幂等恢复持仓/流水/分组 + 测试 | — |

### 阶段 7.4：账户备注修复

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.4.1 账户备注保存修复 | ✅ | architect + invest-logic | updateAccount 不再连带清 note + 支持清空备注 + 测试 | 2026-08-10 |
| 7.4.2 账户卡展示备注 | ✅ | invest-ui | accounts_page 卡片显示备注 + 测试 | 2026-08-10 |
| 7.4.3 备注清空优先级返工 | ✅ | architect + invest-logic | clearNote 优先于 clearMetadataFields + 非卡账户清空测试 | 2026-08-10 |
| 7.4 PM 复审合入 | ✅ | PM | HANDOFF 记录 + commit + Android APK 编译 | 2026-08-10 |

### 阶段 7.3：表外/受托账户

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.3.1 表外账户数据层 | ✅ | architect + invest-logic | accounts.is_off_balance + schema v38 + 统计/AI 排除 + 测试 | 2026-08-09 |
| 7.3.2 表外账户 UI | ✅ | invest-ui | 账户编辑开关 + 资产页标识/分区 + l10n + 测试 | 2026-08-10 |
| 7.3.3 AI 上下文排除表外账户 | ✅ | invest-logic | FinancialAnalystContext 排除 off_balance + 测试 | 2026-08-10 |
| 7.3.4 全流程测试与实机验证 | ⬜ 待实机验证 | qa + invest-logic + invest-ui | 迁移/统计/AI/UI 测试 + 实机验证 | — |

### 阶段 7.2：AI 财务分析师

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.2.1 财务上下文服务 | ✅ | architect + invest-logic | FinancialAnalystSnapshot + 账本/投资摘要 + 测试 | 2026-08-09 |
| 7.2.2 分析师提示词 + 意图路由 | ✅ | invest-logic | 分析师系统提示 + 记账/分析/对话路由 + 测试 | 2026-08-09 |
| 7.2.3 投资分析快捷指令 + 数据范围提示 | ✅ | invest-logic + invest-ui | 投资概览/持仓分析指令 + AI 页入口与范围提示 | 2026-08-09 |
| 7.2.4 全流程测试与实机验证 | ⬜ 待实机验证 | qa + invest-logic + invest-ui | 上下文/路由/指令/UI 测试 + 实机验证 | — |

### 阶段 7.1：小组件显示修复 + App 更名

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.1.1 收支速览溢出 + 大中小预览比例修复 | ✅ | invest-ui + qa | GlanceView 溢出修复 + 预览缩放修正 + 测试 | 2026-08-09 |
| 7.1.2 App 更名「记账助手」 | ✅ | invest-ui + invest-logic | l10n/Android 桌面名/分享文案全量更名 + 测试 | 2026-08-09 |

### 阶段 7.0：原开发者内容清理 + 小组件排查

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 7.0.1 关于/更新日志/隐私政策归属清理 | ✅ | invest-ui | 自用声明 + 删社媒/支持开发 + 本地更新日志/隐私页 + 测试 | 2026-08-09 |
| 7.0.2 更新链路与全仓原仓库链接替换 | ✅ | invest-logic | 更新检测/下载/反馈改指向自身 fork + 全仓残留链接清理 | 2026-08-09 |
| 7.0.3 小组件 Bug 排查修复 | ✅ | qa + invest-ui | 复现定位 + 修复 + 测试 | 2026-08-09 |
| 7.0.4 教程类内容本地化（beejz.com） | ✅ | invest-ui + invest-logic | 帮助中心/云同步/AI 教程改为本地静态页 + 扫描测试 | 2026-08-09 |

### 阶段 6.13：持仓基金代码修复与防错

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.13.1 持仓信息编辑数据层 | ✅ | architect + invest-logic | updateHoldingInfo + 6位校验 + 测试 | 2026-08-08 |
| 6.13.2 基金代码校验 + 编辑入口 | ✅ | invest-ui | 明细编辑入口 + 6位校验 + 测试 | 2026-08-08 |
| 6.13.3 刷新失败反馈 | ✅ | invest-logic + invest-ui | NavRefreshResult + skippedCodes + SnackBar 提示 | 2026-08-09 |
| 6.13.4 删除初始登记/持仓入口 | ✅ | architect + invest-logic + invest-ui | deleteHolding + 删除流水/持仓入口 + 测试 | 2026-08-08 |
| 6.13.5 明细页记一笔快速入口 | ✅ | invest-ui | PrimaryHeader + 跳转编辑器 + 测试 | 2026-08-08 |
| 6.13.6 审查返工：批次流水删除防护 + 附件清理 | ✅ | invest-ui + architect/invest-logic | batch 删除拦截 + deleteHolding 附件清理 + 测试 | 2026-08-08 |

### 阶段 6.12：净值日期显示

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.12.1 净值日期数据层 | ✅ | architect + invest-logic | FundNavQuote + schema v37 navDate + 持久化 + 测试 | 2026-08-08 |
| 6.12.2 净值日期 UI 展示 | ✅ | invest-ui | 卡片/明细/初始持仓显示净值日期 + 测试 | 2026-08-08 |

### 阶段 6.11：天天基金净值自动刷新

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.11.1 NavFetchService | ✅ | invest-logic | 天天基金解析 + 测试 | 2026-08-08 |
| 6.11.2 刷新节流 + 接线 | ✅ | invest-logic | refreshNavsForLedger + 15min | 2026-08-08 |
| 6.11.3 持仓页刷新入口 | ✅ | invest-ui | 下拉/进入自动刷新 + 提示 | 2026-08-08 |
| 6.11.4 进入持仓页刷新时机 | ✅ | invest-ui | tab 切换触发 + 测试 | 2026-08-08 |
| 6.11.5 净值数据源切换（fundgz 失效） | ✅ | PM | 腾讯行情主源 + pingzhongdata 兜底 + 测试 | 2026-08-08 |

### 阶段 6.10：洞察页排行金额精度

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.10.1 排行金额 2 位小数 | ✅ | invest-ui | category_rank_row decimals=2 + 测试 | 2026-08-08 |

### 阶段 6.9：账本账户隔离（已取消）

| 任务 | 状态 | 负责人 | 说明 | 日期 |
|---|---|---|---|---|
| 6.9 账本账户隔离 | ❌ 已取消 | PM | 保持账户跨账本共享，测试数据由用户自行控制 | 2026-08-07 |

### 阶段 6.8：转换页确认固定底部 + 间距放宽

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.8.1 确认按钮固定底部 | ✅ | invest-ui | 底部固定栏 + 测试 | 2026-08-07 |
| 6.8.2 卡片/行距放宽 | ✅ | invest-ui | A/B/C 间距调整 | 2026-08-07 |

### 阶段 6.7：转换记账口径修正 + 支持新目标基金 + 退回账户加载容错

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.7.1 转换记账口径修正 | ✅ | invest-logic | sell/buy amount=0 + 测试 | 2026-08-07 |
| 6.7.2 转换到新目标基金 | ✅ | invest-logic + invest-ui | convert 查找/创建 + 手填提交 | 2026-08-07 |
| 6.7.3 退回账户加载错误处理 | ✅ | invest-ui | catch + 重试 | 2026-08-07 |
| 6.7.4 转换记录展示金额修复 | ✅ | invest-ui | holding_detail 份额×净值展示 | 2026-08-07 |
| 6.7.5 转换提交校验传参修复 | ✅ | invest-ui | validateConvert 传目标基金 + 回归测试 | 2026-08-07 |

### 阶段 6.6：基金转换页面 1x4 组件化

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.6.1 转出/转入组件 A/B | ✅ | invest-ui | A1-A3 + B1-B3 布局 | 2026-08-07 |
| 6.6.2 手续费/退回组件 C | ✅ | invest-ui | 三行输入/下拉 | 2026-08-07 |
| 6.6.3 确认组件 D + AppBar 清理 | ✅ | invest-ui | 确认下移 + 顶部清理 | 2026-08-07 |

### 阶段 6.5：遗留修复 + 基金转换确认记账

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.5.1 转换弹窗加载失败提示 | ✅ | invest-ui | convert 错误态 + 重试 | 2026-08-06 |
| 6.5.2 删除旧记录保留手动净值 | ✅ | invest-logic | recompute 保留 currentNav + 测试 | 2026-08-06 |
| 6.5.3 clearNote 写 NULL | ✅ | invest-logic | updateTransaction NULL + 测试 | 2026-08-06 |
| 6.5.4 转换确认记账 + 退回金额/账户 | ✅ | invest-logic + invest-ui | convert 三笔记账 + 退回 UI | 2026-08-06 |

### 外部审查：kimi 全量审查（2026-08-06，待 PM 审查分派）

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| REV-1 代码审查（逻辑层 + UI 层） | ✅ 已分派6.4 | kimi | docs/HANDOFF.md 最新条目：🔴6 / 🟡7 / 🔵4 | 2026-08-06 |
| REV-2 前端美化分析 | ✅ 已分派6.4 | kimi | 同上：Quick Wins Q1-Q7 + 长期项 L1-L5 | 2026-08-06 |
| REV-3 6.4 修复复审 | ✅ 验收通过 | kimi | docs/HANDOFF.md 复审条目：12 项全 ✅ + 3 项轻微遗留 | 2026-08-06 |

### 阶段 6.4：外部审查修复（kimi REV-1）

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.4.1 投资账户回退查询修复 | ✅ | invest-logic | ledgerId + limit(1) + 测试 | 2026-08-06 |
| 6.4.2 删除投资交易联动重算 | ✅ | architect + invest-logic | deleteTransaction 重算 + 测试 | 2026-08-06 |
| 6.4.3 投资校验/事务/孤儿/备注 | ✅ | invest-logic + invest-ui | 弹窗校验 + service 校验 + 事务 | 2026-08-06 |
| 6.4.4 分隔线暗黑 + 控制器 dispose | ✅ | invest-ui | PopupMenuDivider + dispose | 2026-08-06 |
| 6.4.5 首页统计刷新 + 美化速赢 | ✅ | invest-ui | statsRefresh + border/净资产色 | 2026-08-06 |
| 6.4.6 6.4.3 UI 补完 + 悬空冒号 | ✅ | invest-ui | convert/编辑弹窗校验 + batch 防护 + clearNote UI | 2026-08-06 |

### 阶段 6.3：首页/资产/记账 UI 优化

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.3.1 分组胶囊最小尺寸 | ✅ | invest-ui | holdings_list chip minWidth | 2026-08-06 |
| 6.3.2 排序下拉菜单 | ✅ | invest-ui | sort dropdown + 黑色分隔 | 2026-08-06 |
| 6.3.3 资产概览红绿分行 | ✅ | invest-ui | home asset two rows | 2026-08-06 |
| 6.3.4 周期统计两行 | ✅ | invest-ui | period stats + 时间范围 | 2026-08-06 |
| 6.3.5 删除月度分类占比 | ✅ | invest-ui | home cleanup | 2026-08-06 |
| 6.3.6 时间选择器去秒 | ✅ | invest-ui | wheel time minute-only | 2026-08-06 |

### 阶段 6.2：持仓排序 + 基金分组

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.2.1 持仓排序 | ✅ | invest-logic + invest-ui | sort provider + 排序行 | 2026-08-05 |
| 6.2.2 基金分组 | ✅ | invest-logic + invest-ui | schema v36 + 分组 UI | 2026-08-05 |
| 6.2.3 账本切换分组重置 + 弹窗加载 | ✅ | invest-logic + invest-ui | provider 重置 + dialog future | 2026-08-05 |
| 6.2.4 Notifier state 暴露方式修正 | ✅ | invest-logic + invest-ui | select/reset 方法 + 调用点 | 2026-08-05 |

### 阶段 6.1：投资买入调优 + 资产构成布局 + 我的页清理

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.1.1 买入三字段 + Decimal | ✅ | invest-logic + invest-ui | buy_dialog/service/repo/tests | 2026-08-05 |
| 6.1.2 资产构成布局微调 | ✅ | invest-ui | accounts/chart 布局 | 2026-08-05 |
| 6.1.3 我的页清理排序 | ✅ | invest-ui | mine_page 精简重排 | 2026-08-05 |

### 阶段 6.0：首页/明细导航修正 + 资产页视觉修正

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 6.0.1 明细入口回归底部导航 | ✅ | invest-ui | app.dart 导航 + 首页删明细卡 + 列表返回 | 2026-08-05 |
| 6.0.2 首页排版 + 资产概览改造 | ✅ | invest-ui | home_page 净资产主视觉 + 可点击 | 2026-08-05 |
| 6.0.3 资产页占比文字下移 | ✅ | invest-ui | asset_composition_chart 布局修正 | 2026-08-05 |
| 6.0.4 账户卡片加边框 | ✅ | invest-ui | _AccountCard 边框 | 2026-08-05 |
| 6.0.5 返回链路补全 + 明细防重复 | ✅ | invest-ui | app.dart PopScope + 防抖 | 2026-08-05 |
| 6.0.6 账户卡边框加深 | ✅ | invest-ui | _AccountCard 边框加深 | 2026-08-05 |

### 阶段 5.9：首页导航回归 + 明细头迁移 + Bento

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.9.1 底部导航恢复 5 项 | ✅ | invest-ui | app.dart tab0 改明细 | 2026-08-04 |
| 5.9.2 明细头迁移首页 | ✅ | invest-ui | home/transaction_list 头部 | 2026-08-04 |
| 5.9.3 首页 Bento 美化 | ✅ | invest-ui | home_page bento 布局 | 2026-08-04 |

### 阶段 5.8：首页 + 资产页视觉简化

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.8.1 账户卡面改白色 | ✅ | invest-ui | _AccountCard 简化 | 2026-08-04 |
| 5.8.2 资产顶部只留饼图 | ✅ | invest-ui | 顶部模块简化 | 2026-08-04 |
| 5.8.3 首页 4 入口 | ✅ | invest-ui | 新首页 + 我的减入口 | 2026-08-04 |

### 阶段 5.7：资产页布局细节

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.7.1 图表高度 + 金额标注 | ✅ | invest-ui | accounts_page 图表调整 | 2026-08-04 |
| 5.7.2 类型小计右对齐 | ✅ | invest-ui | 资产/负债标签 + 顺序 | 2026-08-04 |
| 5.7.3 列表底部留白 | ✅ | invest-ui | padding 加大 | 2026-08-04 |

### 阶段 5.6：记账/账户/资产体验改进

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.6.1 小键盘完成仅确认金额 | ✅ | invest-ui | amount_editor_sheet 调整 | 2026-08-03 |
| 5.6.2 再记一笔先保存 | ✅ | invest-ui | tx_entry/transfer 保存逻辑 | 2026-08-03 |
| 5.6.3 账户不计入资产开关 | ✅ | invest-logic | schema v35 + UI + 计算排除 | 2026-08-03 |
| 5.6.4 资产界面布局优化 | ✅ | invest-ui | 图表固定+类型小计+底部余量 | 2026-08-03 |
| 5.6.5 账户自定义 logo | ✅ | invest-ui | 上传图标 + 显示 | 2026-08-03 |

### 阶段 5.5：转账独立 + 记账视觉 + 日期选择器

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.5.1 转出/转入独立选择 | ✅ | invest-ui | transfer_form 拆独立 | 2026-08-03 |
| 5.5.2 反转按钮固定 | ✅ | invest-ui | 账户行布局 | 2026-08-03 |
| 5.5.3 转出/转入灰字标签 | ✅ | invest-ui | 两格布局 | 2026-08-03 |
| 5.5.4 字段行分隔线 | ✅ | invest-ui | 记账页 Divider | 2026-08-03 |
| 5.5.5 日期选择器周几+范围 | ✅ | invest-ui | wheel_date_picker 升级 | 2026-08-03 |

### 阶段 5.4：界面布局固定

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.4.1 底部导航不遮挡内容 | ✅ | invest-ui | app.dart extendBody 固定占位 | 2026-08-03 |
| 5.4.2 明细/洞察/我的适配 | ✅ | invest-ui | 全局 extendBody=false | 2026-08-03 |
| 5.4.3 投资组合固定占位 | ✅ | invest-ui | holdings_list 固定头部 | 2026-08-03 |
| 5.4.4 导入初始持仓移顶部 | ✅ | invest-ui | header actions | 2026-08-03 |

### 阶段 5.3：记账/资产体验改进

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.3.1 转账账户反转按键 | ✅ | invest-ui | transfer_form ⇄ 交互 | 2026-08-02 |
| 5.3.2 日期显示周几 | ✅ | invest-ui | 日期格式化 | 2026-08-02 |
| 5.3.3 再记一笔 + 保存 | ✅ | invest-ui | 底部按钮双模式保存 | 2026-08-02 |
| 5.3.4 删除蜜蜂家当推荐 | ✅ | invest-ui | accounts_page 清理 | 2026-08-02 |
| 5.3.5 资产金额精度 | ✅ | invest-logic | 金额2位/基金净值4位/无万单位 | 2026-08-02 |

### 阶段 5.2：转账模式体验统一

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.2.1 转账金额独立填写 | ✅ | invest-ui | TransferForm 金额行 | 2026-08-02 |
| 5.2.2 转账账户抽屉分格 | ✅ | invest-ui | 账户抽屉公共组件 + 转账行 | 2026-08-02 |

### 阶段 5.1：记账界面体验优化

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.1.1 备注弹框报错修复 | ✅ | invest-ui | tx_entry_form 修复 | 2026-08-02 |
| 5.1.2 备注行内填写 | ✅ | invest-ui | tx_entry_form 备注行 | 2026-08-02 |
| 5.1.3 金额行 + 字段顺序 | ✅ | invest-ui | 金额-分类-账户-时间-标签-备注 | 2026-08-02 |
| 5.1.4 标签行 | ✅ | invest-ui | TagSelector 接入 | 2026-08-02 |
| 5.1.5 小键盘剥离非金额功能 | ✅ | invest-ui | amount_editor_sheet 精简 | 2026-08-02 |
| 5.1.6 账户显示余额 | ✅ | invest-ui | 账户抽屉一行一账户+余额 | 2026-08-02 |
| 5.1.7 分类/账户管理快捷入口 | ✅ | invest-ui | 抽屉顶部编辑/管理按钮 | 2026-08-02 |

### 阶段 5.0：记账账户放开 + 记账页重构

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.0.1 转账放开应收款/贷款 | ✅ | invest-ui | transfer_form 账户过滤修正 | 2026-08-02 |
| 5.0.2 收支放开账户限制 | ✅ | invest-ui | account_selector/picker 修正 | 2026-08-02 |
| 5.0.3 记账页 UI 重构 | ✅ | UI 工程师 | transaction_editor 新布局 | 2026-08-02 |

### 阶段 4.9：账户创建与净值趋势优化

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 4.9.1 虚拟账户不显示卡信息 | ✅ | invest-ui | account_edit_page 条件修正 | 2026-08-01 |
| 4.9.2 应收款改初始资金语义 | ✅ | invest-ui + invest-logic | 估值类型拆分 + 详情页去更新估值 | 2026-08-01 |
| 4.9.3 初始资金日期字段 | ✅ | architect + invest-ui | schema v34 + UI 日期选择 | 2026-08-01 |
| 4.9.4 净值趋势按初始日期 | ✅ | invest-logic | getAccountDailyBalances 修正 | 2026-08-01 |

### 阶段 4.8：遗留问题修复

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 4.8.1 初始持仓重复登记报错 | ✅ | invest-logic | createInitialHolding 复用/新建 | 2026-08-01 |
| 4.8.2 流水过滤 excludeFromStats | ✅ | invest-logic | 流水查询过滤 | 2026-08-01 |
| 4.8.3 删除 GitHub star + 更多产品 | ✅ | invest-ui | mine_page + about_page 清理 | 2026-08-01 |

### 阶段 4.7：投资模块体验修正

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 4.7.1 导入按钮常驻 | ✅ | invest-ui | holdings_list_page 顶部按钮 | — |
| 4.7.2 交易记录编辑 | ✅ | invest-logic + invest-ui | 编辑弹窗 + Repository 方法（返工：保存后事务内重算持仓） | 2026-08-01 |
| 4.7.3 初始持仓标记登记 + 不进流水 | ✅ | invest-logic | investType + excludeFromStats | 2026-08-01 |
| 4.7.4 投资账户不手动估值 | ✅ | invest-ui | account_edit_page 修改 | — |
| 4.7.5 买卖改为转账 | ✅ | invest-logic | buy/sell 交易类型修改（返工：事务内余额联动 + 持仓归属修正） | 2026-08-01 |

### 阶段 4.6：投资逻辑修复 + 初始持仓导入

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 4.6.1 账户资产/负债分组修复 | ✅ | invest-ui | accounts_page 修正 | 2026-07-31 |
| 4.6.2 初始持仓导入 | ✅ | invest-logic + invest-ui | Repository + Service + UI | 2026-07-31 |
| 4.6.3 手续费计入成本 | ✅ | invest-logic | repo 修正 | 2026-07-31 |

### 阶段 4.5：账户体系改造

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 4.5 账户体系改造 | ✅ | architect + invest-ui | 7 类型 + icon字段 + SVG清理 + UI合并 | 2026-07-31 |

---

### 阶段 5：测试 + 上线

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 5.1 全流程手工测试 | ✅ | qa | 验证报告 | 2026-07-30 |
| 5.2 单元测试补充 | ✅ | qa | 测试覆盖率 | 2026-07-30 |
| 5.3 旧数据迁移脚本 | ✅ | migration | 迁移工具 | 2026-07-30 |

### 阶段 4：特色搬运

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 4.1 Excel 导入 | ✅ | integration | excel_import_service.dart + xlsx_reader 已接线 | 2026-07-29 |
| 4.2 日历视图 | ✅ | invest-ui | 日历事件 provider + 彩色圆点渲染 + 事件摘要弹窗 | 2026-07-29 |
| 4.3 设置页 + WebDAV | ✅ | integration | WebDAV已有云备份区 + 账户总览入口 | 2026-07-29 |
| 4.4 时间选择器 | ✅ | integration | BeeCount 已有 WheelTimePicker | 2026-07-29 |

### 阶段 3：投资 UI 层

| 修复任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 3.7 买入流程修复（FAB+holdingId+资金来源） | ✅ | invest-ui | app.dart + buy_dialog | 2026-07-31 |
| 3.8 买入流程修复 2（FAB遮挡+addTransaction不存在） | ✅ | invest-ui | holdings_list_page + app.dart + buy_dialog | 2026-07-31 |
| 3.9 FAB 位置修正（提升到外层 Stack+Positioned） | ✅ | invest-ui | app.dart + holdings_list_page | 2026-07-31 |

| 修复任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 3.6 持仓列表空态加买入入口 | ✅ | invest-ui | FAB + 空态按钮 | 2026-07-30 |

| 修复任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
|| 3.5 页面连线（卡片→明细→弹窗） | ✅ | invest-ui | 3 处 wiring 修复 | 2026-07-29 |

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 3.1 持仓列表页 | ✅ | invest-ui | 页面代码 | 2026-07-29 |
| 3.2 持仓明细页 | ✅ | invest-ui | 页面代码 | 2026-07-29 |
| 3.3 操作弹窗（买入/卖出/转换） | ✅ | invest-ui | 弹窗组件 | 2026-07-29 |
| 3.4 首页 5 Tab 导航 | ✅ | invest-ui | 导航改造 | 2026-07-29 |

### 阶段 2：投资逻辑层

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 2.1 InvestmentService | ✅ | invest-logic | Service 封装 | 2026-07-29 |
| 2.2 Riverpod Provider | ✅ | invest-logic | Provider 定义 | 2026-07-29 |
| 2.3 单元测试 | ✅ | invest-logic | 16 测试全通过 | 2026-07-29 |

### 阶段 1：投资数据层

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 1.1 InvestmentHoldings 表 | ✅ | architect | db.dart 定义 | 2026-07-29 |
| 1.2 Transaction 投资字段 | ✅ | architect | db.dart 扩展 | 2026-07-29 |
| 1.3 Schema 迁移 v31→v32 | ✅ | architect | 迁移代码 | 2026-07-29 |
| 1.4 InvestmentRepository | ✅ | architect | 4 个核心事务 | 2026-07-29 |
| 1.5 Repository 单元测试 | ✅ | architect | 13 测试全通过 | 2026-07-29 |

### 阶段 0：Fork + 瘦身

| 任务 | 状态 | 负责人 | 产出物 | 完成日期 |
|---|---|---|---|---|
| 0.1 Fork 仓库 + 改包名 | ✅ | 用户 | 私有仓库 | — |
| 0.2 删除 iOS 代码 | ⬜ | — | iOS 残留清除 | — |
| 0.3 瘦身（共享账本/IAP/营销/多语言） | ⬜ | — | 工程瘦身完成 | — |
| 0.4 添加 Windows 支持 | ⬜ | — | Windows 可编译 | — |
| 0.5 验证基线 | ⬜ | — | 测试全过 | — |

## 任务分派工作流

### PM 分派任务

1. PM 在本文件任务板中将任务标记为 🔄 进行中，填写负责人
2. PM 创建新线程，传入：
   - 要读的文档路径（本文件 + PROJECT_PLAN.md）
   - 具体任务描述
   - 相关的文件引用
   - 已知约束/架构决策

### 角色执行任务

1. 读取 docs/TEAM.md → 确认自己的任务
2. 读取 docs/PROJECT_PLAN.md → 理解背景
3. 读取任务相关的现有代码 → 理解上下文
4. 实现任务
5. 更新本文件任务板（标记 ✅）

### PM 审查合入

1. 审查代码（按 bug/风险/测试缺失/风格问题排序）
2. 要求修复问题
3. 确认修复后合入主分支
4. 更新任务板 → 进入下一任务

---

## 图像识别规范（读图/截图/参考图）

当任务涉及图片（用户发图、截图、本地图片文件、URL、图表/UI 参考图）且当前模型不支持图像输入时，**必须使用 deepseek-vision-skill** 获取图片描述，再基于描述继续工作。

**触发场景**：
- 用户引用本地图片（如 D:/Users/wanji/Downloads/首页.jpg）
- 需要 OCR / 截图内容 / UI 视觉检查
- 消息中出现「image content omitted because you do not support image input」

**用法**（脚本在 C:/Users/wanji/.codex/skills/deepseek-vision-skill/scripts/describe-image.js）：
- 本地图片：node "C:/Users/wanji/.codex/skills/deepseek-vision-skill/scripts/describe-image.js" "图片路径"
- 用户刚发的图：node "...describe-image.js" --latest
- 带问题：node "...describe-image.js" --prompt "具体问题" "图片路径"

**要求**：
- 用脚本输出的事实描述，OCR 文本必须原样引用，不得臆测
- 脚本依赖网络与 API key（config.json）；失败时告知用户并提供替代方案
- 图片描述仅用于理解需求，不写入最终交付物（除非用户要求）
---

## 跨会话交接协议

当一个线程完成任务后交接给另一个线程：

1. **git 提交**：所有变更必须提交，commit message 写明做了什么
2. **更新任务板**：本文件的任务板标记 ✅
3. **写交接记录**：在 docs/HANDOFF.md 模板正下方插入新记录（最新在前，按日期倒序）

### HANDOFF.md 编辑铁律（防止历史丢失）

- **只增不减**：任何编辑只允许「新增」内容，绝不允许删除/覆盖已有记录
- **禁止整文件重写**：不得基于过期快照用 Python/PowerShell 重写整个文件；新增记录用唯一锚点（新记录的「**任务**：5.x」标题）定位插入，或只在模板正下方 prepend 一个固定块
- **禁止模糊正则范围替换**：不得用匹配 ## 2026-08-02 / --- 这类重复标题的正则做范围删除或替换
- **编辑后自检**：每次改完 HANDOFF 立即运行
  git diff -- docs/HANDOFF.md
  除了 --- a/docs/HANDOFF.md 这一行外，不允许出现任何以 - 开头的内容行；出现即说明删除了历史，必须恢复

### 交接记录模板

```
## {日期}

**移交角色**：[角色名]
**接收角色**：[角色名]
**完成工作**：
- 改了哪些文件
- 做了什么变更
- 还有什么未完成

**下一个任务需要知道的**：
- 关键决策
- 已知问题
- 要读的代码上下文

**git 状态**：当前分支 [branch name]，已提交 [commit hash]
```




