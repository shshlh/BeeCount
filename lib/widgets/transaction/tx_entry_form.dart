...
class _TxEntryFormState extends ConsumerState<TxEntryForm> {
  ...
  Future<void> _pickAccount() async {
    final account = await _showAccountDrawer(
      context,
...
Future<Account?> _showAccountDrawer(
  BuildContext context, {
...
class _AccountDrawerSheetState extends ConsumerState<_AccountDrawerSheet> {
  ...
  Future<_AccountDrawerData> _load() async {
    ...
    var visible = accounts
        .where((a) =>
            a.currency == currency && isBookingAccountType(a.type))
        .toList();
    final pinnedId = widget.pinnedAccountId;
    if (pinnedId != null && !visible.any((a) => a.id == pinnedId)) {
