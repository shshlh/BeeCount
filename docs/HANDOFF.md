...
**git 状态**：当前分支 main，基线 `3365aa0`，待提交构建记录
...
**完成工作**：
- 审查通过：今日收益状态标签融入今日收益行，三态样式符合 v3；基金列表与详情页视觉已对齐
- 全量 `flutter test`：867 passed / 1 skipped / 0 failed
- 改动文件 analyze 无新增 error
- 合入 main：`ad3c56a`

**待实机验证**：窄屏手机上的金额/状态标签是否溢出，建议真机截图确认。

**git 状态**：当前分支 main，已提交 `ad3c56a`
...
**任务**：7.20 基金模块 v3 UI 改版（状态标签优先）

**设计来源**：`D:/Users/wanji/Downloads/fund_ui_design_v3.md`

**要求**：

7.20.1 今日收益状态标签（优先）：
- 状态标签紧跟「今日收益」百分比后，间距 8px，不单独成行。
- 状态映射：
  - `pending` -> 待更新：背景 `#f0f0f0`，文字 `#999`
  - `partialUpdated` -> 部分更新：背景 `#fff8e1`，文字 `#f9a825`
  - `allUpdated` -> 更新完成：背景 `#f0fff0`，文字 `#51cf66`
  - `nonTradingDay` 保留业务状态，可显示「非交易日」，沿用待更新的灰样式
- 标签字号 10px，圆角 4px，padding 2px 6px。
- 现有独立的「今日净值状态」行移除或融合进今日收益行。

7.20.2 基金列表与详情页视觉对齐（后续）：
- 汇总卡片、基金卡片、详情页 Hero、今日/昨日收益、参数区、交易记录按 v3 布局与颜色规范执行。
- 颜色继续遵循项目语义：正向用红 `#e74c3c`，负向用绿 `#51cf66`。

**注意**：先完成 7.19.5 再进入 7.20.2，避免逻辑修复和视觉重构同时改动同一批组件。

**约束**：全量 `flutter test` 0 failed；改动文件 analyze 无新增 error；HANDOFF/TEAM 只增不减。

**git 状态**：当前分支 main，基线 `9ca8391`，待派工
...
**背景**：8 月 18 日实机测试发现 3 个问题，基线 `e1bc7c5`。

**要求**：

7.19.1 QDII 买入确认净值日期（invest-ui + invest-logic + qa）：
- `BuyDialog` 新增「确认净值日期」选择，默认今天，允许用户改成基金确认的实际净值日期。
- 提交时把该日期传给 `service.buy(..., navDate: selectedNavDate)`，仓库不要再默认用交易时间。
- QDII 通常为 T-2，用户可按确认单选择 8 月 14 日等实际日期。
- 测试：买入传自定义净值日期后，持仓和投资流水的 `navDate` 使用该日期，而非交易发生时间。

7.19.2 编辑模式「再记一笔」覆盖问题（invest-ui + qa）：
- 编辑已有流水时，第一次「再记一笔」应保存当前编辑，随后切换到新建模式；第二次保存必须新增一条流水，不能继续覆盖原流水。
- `TxEntryForm` 与 `TransferForm` 都要处理；不要直接依赖固定的 `widget.editingTransactionId`。
- 测试：编辑流水 A -> 再记一笔 -> 填流水 B -> 保存后，A 与 B 同时存在且内容正确。

7.19.3 今日/昨日收益按“更新日”对齐（invest-logic + invest-ui + qa）：
- 单基金今天未更新时，今日收益/今日涨跌应为 `null`，UI 显示 `--` 或「待更新」，不得把最近一次旧净值差当作今日收益。
- 今天已更新时，今日收益 = 最新净值差，昨日收益 = 前一次净值差；QDII 今天更新但净值日滞后，仍按今天更新处理。
- 持仓汇总卡片与单基金卡片都要同步；部分/全部更新状态继续按今天的实际更新情况计算。
- 测试：今天未更新、今天已更新、QDII 滞后更新三种场景。

**约束**：全量 `flutter test` 0 failed；改动文件 analyze 无新增 error；HANDOFF/TEAM 只增不减。

**git 状态**：当前分支 main，基线 `e1bc7c5`，待派工
...
**完成工作**：
- 7.17.9 修复通过：`advancedCount` 只统计非货币基金，`totalAdvancedCount` 保留全部前进数；普通基金未更新、货币基金已更新时正确显示「部分更新」
- 货币基金抓取失败不再进入 `skippedCodes`
- 已确认前序修复有效：天天基金 3 档主源、摘要无条件刷新、跨日状态 key
- 全量 `flutter test`：841 passed / 1 skipped / 0 failed；改动文件 analyze 无新增 error
- 合入 main：`72b58d5`

**遗留（P3，非阻塞）**：
- `fund_nav_history` 无物理清理，后续可补 prune
- 收益/涨跌计算仍为 `double`，后续可统一为 `Decimal`
- 仅持有货币基金且全部抓取失败时，可能因节流兜底分支保留上一次状态，后续可优化

**git 状态**：当前分支 main，已提交 `72b58d5`
...
**完成工作**：
- 全新账本单独导入配置 YAML，账户数据成功显示并绑定正确账本，账户管理页不再为空

**git 状态**：当前分支 main，无代码变更
...
**核心口径**：收益/涨跌幅用「更新日」定义，日期统一标「今天 / 昨天」，不标净值日。QDII 净值日滞后 1~2 天，但每天 20:00 后拉到前进的净值即算「已更新」。

**要求**：

7.17.1 净值历史表（architect）：
- 新表 `fund_nav_history(fund_code, nav_date, unit_nav, updated_at)`，主键 `(fund_code, nav_date)`，schema v40 → v41。
- 每基金按净值日 upsert，保留最近若干条；供「最新 / 次新 / 第三新」三档净值计算。

7.17.2 净值抓取扩展（invest-logic）：
- `NavFetchService` 抓取最近 3 档净值：天天基金 `Data_netWorthTrend` 取末尾 3 条；腾讯行情兜底（仅最新一条 + 日增长率）。
- 返回多档净值，写历史表；腾讯 `fields[7]` 日增长率仅作兜底校验，不替代净值差计算。

7.17.3 收益/涨跌幅计算（invest-logic）：
- 今日收益 = `(最新 - 次新) × 份额`；昨日收益 = `(次新 - 第三新) × 份额`。
- 今日涨跌幅 = `(最新 - 次新) / 次新`；昨日涨跌幅 = `(次新 - 第三新) / 第三新`。
- 纯函数 + 单测；货币基金排除（返回「不适用」）。

7.17.4 非交易日判断 + 节假日数据源（invest-logic）：
- 新建 `HolidayCalendar`：周末 + 内置 2026/2027 法定节假日表，提供 `isTradingDay(date)` / `isHoliday(date)`。
- 记账日历可选复用同一数据源做节假日高亮（非本次必需）。

7.17.5 刷新与部分/全部更新（invest-logic + invest-ui）：
- 0:01~20:00 今日收益显示「待更新」，不拉净值；20:00 后进入持仓页即拉，之后沿用 15 分钟节流轮询，到 23:59 或全部更新后停。
- 「已更新」判断 = 该基金 `nav_date` 比上次前进；「部分更新 / 全部更新」据此汇总。
- 非交易日（周末/节假日）显示「非交易日」。

7.17.6 持仓卡片 UI（invest-ui）：
- 汇总卡片与单基金卡片展示「今日收益 / 昨日收益」+「今日涨跌 / 昨日涨跌」，括号标日期（如 今日收益 2026.8.18）。
- 汇总卡片显示「部分更新 / 全部更新 / 待更新 / 非交易日」状态；货币基金收益/涨跌显示「--」。

7.17.7 测试（qa）：
- 收益/涨跌幅计算（含 QDII 滞后场景）、非交易日判断、净值历史 upsert、部分/全部更新、UI 状态。

**约束**：全量 `flutter test` 0 failed；改动文件 analyze 无新增 error；HANDOFF/TEAM 只增不减。

**git 状态**：当前分支 main，基线 `0e2e37a`，待派工
...
**完成工作**：
- 审查通过：账户结构归配置 YAML（补 7 字段 + 手写 buffer 补写），配置导入绑定目标账本并按 (ledgerId, name) 复用/更新；投资 CSV 移除【账户】段、按名匹配
- 全量 `flutter test`：815 passed / 1 skipped / 0 failed；改动文件 analyze 无 error
- 合入 main：`77580f6`

**git 状态**：当前分支 main，已提交 `77580f6`
...
**要求**：
- `lib/pages/account/account_detail_page.dart` 的 `_TransactionTile` 移除账本名称小标签（当前账户跟账本走，详情页流水同账本，标签冗余）；同时清理不再使用的 `ledgers` 相关取数。
- 投资流水（`investType != null && holdingId != null`）在该页补基金代码/名称标识，对齐明细页 `transaction_list.dart` 的 `fundLabel` 口径（用 holdingId → fundCode/fundName 映射）。
- 补/更新 widget 测试：投资流水显示基金标识、普通流水不显示、账本标签不再出现。
- 全量 `flutter test` 0 failed；改动文件 analyze 无新增 error；HANDOFF/TEAM 只增不减。

**git 状态**：当前分支 main，基线 `a580438`，待派工
...
**完成工作**：
- 审查通过：投资 CSV【账户】段补全初始资金日期/备注/开户行/卡号后四位/信用额度/账单日/还款日，导出导入字段对齐，空值保持 null
- 全量 `flutter test`：814 passed / 1 skipped / 0 failed；改动文件 analyze 无 error
- 合入 main：`097ae93`

**git 状态**：当前分支 main，已提交 `097ae93`
...
**完成工作**：
- 审查通过：账户详情页流水 tab 全部/支出/收入，默认全部；全部模式逐笔余额按当前余额倒序反推，估值账户恒为缓存市值，分页精确
- 全量 `flutter test`：814 passed / 1 skipped / 0 failed；改动文件 analyze 无 error
- 合入 main：`cb22e6f`

**说明**：信用卡详情页原来固定展示全部流水，现与其它账户统一为三 tab（可接受，非 bug）。

**git 状态**：当前分支 main，已提交 `cb22e6f`
...
**完成工作**：
- 7.14.1 账户详情页流水 tab 改为「全部 / 支出 / 收入」，默认「全部」；「全部」对应 `flow=null` 展示全部收支/转账，分类分布图在「全部」模式下隐藏
- 7.14.2 逐笔余额：「全部」模式每笔流水金额下方灰色小字显示「余额 xx.xx」；口径 = 当前余额按时间倒序反推（收入 + / 支出 - / 转出 - / 转入 +），估值/投资账户恒为缓存市值；分页下也精确（不依赖已加载子集）
- 7.14.3 测试：新增 tab→flow 映射与逐笔余额纯逻辑单测（含转出/转入/估值账户）、账户详情页 widget 测试（默认全部无分布图、切支出/收入显示分布图、逐笔余额 1200/700/800 正确）
...
**任务**：7.13.6/7.13.7 复审合入

**完成工作**：
- 审查通过：普通 CSV 按 (ledgerId, name) 去重；投资 CSV 先按【账户】段预建全部账户；搜索页转账显示对齐流水页
- 全量 `flutter test`：810 passed / 1 skipped / 0 failed；改动文件 analyze 无 error
- 合入 main：`932886c`

**遗留（P3，非阻塞）**：
- 搜索页转账 `accountName` 用 `'${a?.name ?? ''} → ${b?.name ?? ''}'`，一端为 null 时会显示空箭头；流水页对 null 是整段不显示。普通转账两端账户都在，影响极小，后续可对齐。

**git 状态**：当前分支 main，已提交 `932886c`
...
**完成工作**：
- 审查通过：搜索入口迁移、三层筛选 + 转账搜索、数据管理三分组重构；返工项（分组顺序 + 空态文案）均已修正
- 全量 `flutter test`：807 passed / 1 skipped / 0 failed；改动文件 analyze 无 error
- 合入 main：`ab27f22`

**git 状态**：当前分支 main，已提交 `ab27f22`
...
**完成工作**：
- 数据管理页分组顺序改为「分类与标签 → 导入与导出 → 空间与清理」，更新注释编号，分组内容/入口行为不变
- 三层筛选弹窗未选类型时显示中性提示「请选择收入/支出/转账」，仅选转账时显示「转账无分类」
- 更新 widget 测试：分组顺序断言（y 轴递增）与空态文案断言（未选类型 / 选转账两种状态）

**验证**：
- 全量 `flutter test`：807 passed / 1 skipped / 0 failed
- 改动文件 `dart analyze` 无新增 issue；全仓 analyze 859 项既有 info/warning，无 error

**git 状态**：当前分支 main，7.13 整体改动（含本返工）仍未提交，待 PM 审查合入
...
**任务**：7.12 派工补充（问题1 orphan 清理 + 问题2/3 类型恢复细化）

**补充要求**：

7.12.1 追加：
- 删账本 / 清空 / 初始化时，除删除 `ledger_id = 账本id` 的账户外，同时清理 orphan 账户（`ledger_id = 0` 或指向已不存在账本且无任何关联的账户），避免脏数据残留被全局统计继续计入净资产。
- 首页净资产按账本过滤是「治标」，orphan 清理是「治本」，两者都要做。

7.12.2 细化：
- 投资 CSV 的【账户】段是账户 type / initialBalance / sortOrder 的唯一权威来源；导入时账户类型以【账户】段为准，**不再用「账户名是否出现在持仓所属账户」这种启发式猜 type**。
- `ensureAccount` 改为「先解析账户段建立 name→(type/初始资金/排序) 映射，再按映射创建」，避免「先到先得」把余额宝这类账户误判为 cash 或 investment。
- 普通 CSV 的储蓄卡/信用卡/投资账户 type 也要能恢复（通过账户段或等价机制），不能全部塌缩成 cash。

**git 状态**：当前分支 main，基线 `c46962b`，待派工补充
...
**决定**：投资流水在明细页单独显示「基金代码/名称」作为次要信息，不写入备注，避免覆盖用户备注。

**要求**：
1. 明细列表预加载 `holdingId → (fundCode, fundName)` 映射；对 `investType != null && holdingId != null` 的流水追加基金标识
2. 基金标识作为独立的次要信息行/小标签，不修改 `note`
3. 投资 CSV 已包含基金代码/名称，继续作为导出归档的唯一投资流水来源
4. 测试：买入/卖出流水显示基金名且备注不被覆盖；转换内部记录不重复
5. 全量 `flutter test` 0 failed，改动文件 analyze 无新增 issue
...
**移交角色**：项目经理（PM）
**接收角色**：invest-logic + qa + architect

**任务**：7.9.1 导出投资与明细一一对应，避免重复

**问题**：普通流水 CSV 用 `transactionsWithCategoryAll`，会包含投资买入/卖出/退回等流水；投资 CSV 又包含这些流水，导致同一笔记录在两个文件重复。同时投资持仓与详情页买卖/转换记录仍应完整导出。

**要求**：
1. 新增/复用查询：普通流水 CSV 排除投资相关流水（`investType != null`、`batchId != null`、`note='基金转换退回'`），投资 CSV 作为投资流水的唯一出处
2. 投资 CSV 保留持仓、投资流水、分组归属；投资流水增加唯一标识（如本地 id / syncId）与转出/转入账户、金额/成本等字段，保证和原明细一一对应
3. 测试：买入/卖出/转换/退回在普通 CSV 与投资 CSV 中只出现一次；持仓与转换内部记录仍完整
4. 全量 `flutter test` 0 failed，改动文件 analyze 无新增 issue
...
**问题**：明细页目前只能通过 Dismissible 单条删除，没有多选/批量删除入口。

**要求**：
1. 明细页增加多选模式：进入选择模式、行前勾选框、全选/取消全选、底部批量删除
2. 数据层支持按本地 id 批量删除（可复用现有 batch 能力；删除后清理标签/附件并重算投资持仓）
3. 删除前二次确认；完成后刷新明细、统计、预算、小组件和同步
4. 测试：批量删除接口、选择模式 UI、投资流水混入批量删除后持仓正确重算
5. 全量 `flutter test` 0 failed，改动文件 analyze 无新增 issue
...
**任务**：7.9.3 账本初始化/清空全部数据

**问题**：当前「清空账本」只删除 transactions，不清投资持仓/分组/分组归属等数据，未达到“初始化账本”预期。

**要求**：
1. 新增/完善 `resetLedger`：删除该账本的 transactions、transaction_tags、附件关联、投资持仓、投资分组及分组归属，并清理对应物理附件；保留账本、账户、分类、标签结构
2. 明确「清空账本」与「删除账本」的区别；在数据管理或账本管理入口提供「初始化账本」并二次确认
3. 清空后同步 `PostProcessor.sync`、清空交易缓存、刷新统计/预算/持仓/小组件
4. 测试：初始化后流水、持仓、分组为空，账户/分类/标签保留；同步变更正确记录
5. 全量 `flutter test` 0 failed，改动文件 analyze 无新增 issue
...
**背景**：
- 当前转换拆成 A「卖出」+ B「买入」两笔共享 `batchId` 的流水，两边各自只显示一侧，且整批编辑/删除被拦截
- B 持仓成本目前按「转入份额 × 转入净值」计算，不区分转入手续费，导致实盘转换出现尾差
- 产品决定：转换记录在 A/B 两页都显示「转换」标签，可从任意一侧编辑/删除整批；新增「转入成本」字段，B 成本与盈亏以实际成本为准

**需求 1：转换记录统一展示/编辑/删除**
- 持仓明细交易行：`batchId != null` 时标签统一显示「转换」，不再显示「卖出/买入」
- 点击任意一侧转换行 → 打开转换编辑页，预填：确认转出份额、确认转出净值、确认转入份额、确认转入净值、转入成本、手续费、退回金额、退回账户、日期、备注
- 保存 = 原子更新整批（A卖出 + B买入 + 关联退回），重算双方持仓与投资账户市值
- 删除任意一侧转换行 = 删除整批（含关联退回记录），重算双方持仓与投资账户市值；提示文案改为「删除完整的转换记录」

**需求 2：新增转入成本字段**
- 新建转换弹窗 B 组件最后一行加「转入成本」（必填、>0），提交时传入 service/repo
- 编辑模式同样可改「转入成本」

**需求 3：B 持仓口径**
- 转换后 B `totalCost += 转入成本`；B `marketValue = totalShares × 最新净值`（转换时即 转入净值 × 转入份额）；持仓盈亏 = marketValue - totalCost
- 转换买入侧交易 `amount` 存「转入成本」；`_recomputeHolding` 转换买入分支改为 `amount > 0 ? amount : shares × nav`
- 手续费建议只存转出侧（`investFee`），避免同一笔费用在两侧重复记录；B 成本不受手续费推导影响
- 退回金额按实际确认值记录；默认自动值建议改为 `转出市值 - 转入成本 - 手续费`（>=0，可手改）

**数据层要求**：
- `convert` / `validateConvert` 增加 `toCost` 参数
- 退回流水也写同一 `batchId`，保证整批删除能带走退回记录
- 新增 `updateConversion(batchId, ...)` / `deleteConversion(batchId)`，或等价 service 方法；编辑时同步更新卖出/买入流水并重算双方持仓
- 旧转换记录兼容：无 `amount`（转入成本）的买入侧回退为 `份额 × 净值`

**测试要求**：
- 仓库/服务测试：B cost = 转入成本、marketValue = toNav × toShares、buy side amount = toCost、refund 带 batchId
- UI 测试：A/B 两页都显示「转换」；从 A 或 B 编辑/删除都能更新/删除整批并重算
- 全量 `flutter test` 0 failed，改动文件 analyze 0 新增 issue

**git 状态**：当前分支 main，HEAD 306eaa8，工作区干净
...
**审查结论**：
- 像素级复核通过：`adaptive_foreground.png` 与用户前景原图一致（bbox 231,224-792,800），`adaptive_monochrome.png` 的 alpha bbox 与前景完全对齐，legacy 为不透明 RGB 且字体笔画已合成（非只剩背景色），背景源图仅 1 种颜色 `#F5F0E8`
- 配置正确：`ic_launcher.xml` 移除 16% inset 以匹配 1:1 预览，`pubspec.yaml` 背景色与注释同步，已删除失效的「legacy 钉死为线上版本」警告
- 未跑 Dart 测试属合理（纯资源/配置变更）；未编译 APK（按用户要求暂停），正式发布前需实机验证桌面图标、圆形遮罩、themed icon 与小组件

**遗留/风险**：
- widget 预览图与 `preview_themed.png` 未在本轮重新生成，实机时需确认小组件预览是否仍显示旧图标
- 用户三张原图仍在仓库外（D:/Users/wanji/Downloads/），如需归档建议后续移入仓库

**git 状态**：当前分支 main，2e3ff11（图标）+ d9fe10f（交接文档）+ 本记录已入库
